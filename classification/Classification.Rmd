...
- likely predictors include
    ...
    - PANSS_Total
    - PatientID??
- we will keep AssessmentID in our dataframe to represent each patient.
- except
    - Individual scores are difficult to understand in terms of how they affect the assessment status. We'll have total scores anyway. When using individual scores as predictors, we sometimes exclude the total PANSS value.
    - Study: no observation in study_E contains the LeadStatus.
- when it comes time to predict, we specify a assessment ID and get a probability of LeadStatus being NOT Passed (Flagged)

### Data

- I will combine A-D as one dataset(combABCD)
- The training dataset will be 75% randomly chosen assessment IDs from combABCD
- The development dataset will be the rest 25% of combABCD.
- The test dataset for kaggle will be StudyE, for which we have no LeadStatus.

### Thoughts/Conclusions
- I start to use Naive Bayes, and then use logistic, and use the LDA, which as expected did similar. QDA was worse, so I choose linear decision boundary.
- It wasn't a good trade-off to increase variation in order to reduce bias by assuming different variances for each predictor in QDA.
- These two were both better than Naive Bayes.
- then I tried SVM, which didn't do better than logistic even with extensive tuning, and gbm and rf also not perform well.GBM and SVM had lower development set loglosses.
